...
signal click_tarsier_area(_viewport, event, _shape_idx)
...
func _on_walking_area_input_event(_viewport, event, _shape_idx):
	if on_area_tarsier:
		click_tarsier_area.emit(_viewport, event, _shape_idx)
		on_area_tarsier = false
	else:
		click_area_input_event.emit(_viewport, event, _shape_idx)

func _on_click_tarsier_input_event(_viewport, _event, _shape_idx):
